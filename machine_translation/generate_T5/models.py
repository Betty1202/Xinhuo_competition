...
class GenerativeT5(torch.nn.Module):
    ...
    def __init__(self, encoder, decoder_with_lm_head, tokenizer, onnx=False, cuda=False, dtu=False):
        super().__init__()
        self.encoder = encoder
    ...
    def forward(self, prompt, max_length, temperature=1., repetition_penalty=1., top_k=50, top_p=0,
        ...
        if not self.dtu:
                ...
                for _ in trange(max_length):
                    if self.onnx:
                        outputs = torch.tensor(
                            self.decoder_with_lm_head.run(None, {"input_ids": generated.cpu().numpy(),
            ...
            with TopsInference.device(0, 0):
                ...
                self.encoder = TopsInference.load(self.encoder)
                self.decoder_with_lm_head = TopsInference.load(self.decoder_with_lm_head)

                new_tokens = torch.tensor(())
                new_logits = []
                generated = torch.tensor(self.tokenizer(prompt)['input_ids'])[:max_context_length - 1].unsqueeze(0)
                generated = F.pad(generated, (0, 256 - len(generated[0])), 'constant', 0).numpy().astype(np.int64)
                temperature = temperature
                # Getting encoder past
                encoder_outputs_prompt = []
                self.encoder.run([generated], encoder_outputs_prompt,
                                 TopsInference.TIF_ENGINE_RSC_IN_HOST_OUT_HOST)
                encoder_outputs_prompt = encoder_outputs_prompt[0].astype(np.float32)
                ...
                for _ in trange(max_length):
                    outputs = []
                    generated_input = F.pad(generated, (0, 256 - len(generated[0])), 'constant', 0).numpy().astype(
                        np.int64)
                    self.decoder_with_lm_head.run(
                        [generated_input, encoder_outputs_prompt], outputs,
                        TopsInference.TIF_ENGINE_RSC_IN_HOST_OUT_HOST)
                    outputs = torch.tensor(outputs[0][0])
                    next_token_logits = outputs[len(generated[0]) - 1, :] / (temperature if temperature > 0 else 1.0)
                    if int(next_token_logits.argmax()) == 1:
                        break
                    new_logits.append(next_token_logits)
                    for _ in set(generated.view(-1).tolist()):
