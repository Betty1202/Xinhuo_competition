...
def caption_image_beam_search(encoder, decoder, image_path, word_map, beam_size=3):
    ...
    image_np = image.numpy().astype(np.float32, order="C")
    encoder_output = []
    encoder.run(image_np, encoder_output,
...
def run(args):
    ...
    with TopsInference.device(0, 0):
        encoder = TopsInference.load('./engines/encoder_default_bs1.exec')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model',
                        default='1BEST_checkpoint_flickr8k_5_cap_per_img_5_min_word_freq.pth.tar',
                        help='path to model')
